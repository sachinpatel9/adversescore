...
Current state (Phase 0–11 complete): Foundation cleanup, drug identity resolution, chunked FDA retrieval, deduplication, label status classification, deterministic ranking engine, consolidation orchestration, persistence layer, agent orchestration with guardrails, document generation, Streamlit UI integration, and comprehensive test suite and multi-drug live validation are all complete.

- **`app.py`** (REBUILT, Phase 10) — Streamlit UI wiring the full Phase 1–9 PSUR consolidation pipeline. Session state: `messages`, `session_id` (UUID per browser session), `in_session_dataset` (Optional `ConsolidationResult`), `consolidation_id` (Optional int, DB row id), `generated_doc_bytes` (Optional bytes, for two-step .docx export). `ConsolidationStore()` constructed fresh on every script rerun (not cached in session_state, since Streamlit reruns entire script on each interaction and long-lived cached SQLite connection risks cross-rerun issues). Sidebar: drug name text input + period selectbox (6mo/1yr/2yr/3yr) + "Run Consolidation" button calls `consolidate_psur()` **directly** (bypasses LLM agent entirely per locked design decision; chat reserved for follow-ups only). On `ConsolidationError`, shows stage-differentiated error message (CLIENT_CONSTRUCTION/IDENTITY_RESOLUTION/RETRIEVAL); error never persisted. "Prior Sessions" section lists past consolidations via `store.list_consolidations()`, resumable via `store.load_consolidation()` (convenience feature, not in original scope but added per Phase 10 planning). Main area (when dataset active): identity resolution feedback card (canonical name, brand/generic names, market authorization date or OTC fallback, resolution confidence badge), data completeness card (Guardrail 5: retrieved/estimated counts, per-chunk truncation flags, dedup stats — all sourced from real `ConsolidationResult` fields, never hardcoded), ranked signal table capped at `TOP_N_NARRATED_SIGNALS` (imported from config, not duplicated hardcoded number) with note that full list lives in exported document, two-step "Generate PSUR Document" → `st.download_button` export flow (generated bytes stored in session_state to survive the rerun the download button triggers; invalidated on every dataset-changing action). Chat interface rewired to call `run_agent_turn()` (Phase 8's pure agent function) on every turn, passing `in_session_dataset` so agent can answer from existing dataset instead of re-fetching (verified against real `orchestrator.py` dataset-reuse mechanism); chat never calls `consolidate_psur()` directly — only sidebar button does. Every chat turn persisted via `store.save_message()` for both user and assistant; if agent's tool-calling produces new dataset mid-conversation (`AgentTurnResult.dataset_changed`), persisted via `store.save_consolidation()` and becomes active dataset. Dead unreachable dark-mode CSS removed. All backend call sites wrapped try/except (no raw Python traceback to end user). HTML-escaping (`html.escape()`) applied to all dynamic values in `st.markdown(..., unsafe_allow_html=True)` blocks (drug names, chunk labels) as defense-in-depth against openFDA API response injection. Bonus fix: `orchestrator.py` system prompt's "NARRATION CAP" section updated (was stale: "document export not exportable yet"; now acknowledges real export button exists while clarifying agent doesn't generate/attach documents). Test counts unchanged: 233 unit, 20 E2E (Phase 10 added no test files per scope doc's explicit "no automated UI testing" statement).
...
- **Phase 10 UI Design: Direct Consolidation Bypass of LLM Agent** — The Streamlit sidebar's "Run Consolidation" action calls `consolidate_psur()` directly, bypassing the multi-turn agent orchestrator entirely. This locked design decision reflects the principle that data-gathering operations (deterministic, repeatable, cacheable via Phase 7 persistence) should be separated from conversational follow-ups (contextual, exploratory). The sidebar button is the primary UX path for launching new PSUR consolidations; the chat interface is reserved exclusively for follow-up questions and exploratory analysis that reuse the cached in-session dataset. This asymmetry prevents redundant FDA queries and ensures reproducibility: the same drug+period always retrieves identical data, regardless of how many conversation turns follow. The agent never auto-launches a new consolidation — only the user's explicit sidebar action triggers one.
- **Phase 10 Prior Session Resumption Feature** — Added to the Streamlit UI as an analyst convenience beyond the original scope doc (which focused on chat follow-ups): a "Prior Sessions" sidebar section lists all past consolidations via `store.list_consolidations()` (lightweight metadata only), allowing a PV analyst to resume a prior session without re-running the FDA pipeline. Clicking a prior consolidation reloads it via `store.load_consolidation()` into the active `in_session_dataset`. This workflow supports common PV analyst patterns (e.g., "I reviewed KEYTRUDA/6mo yesterday — let me pick it back up today and ask new follow-up questions") while reinforcing the persistence layer's value and the practical asymmetry between one-time deterministic data gathering (cached) and unlimited exploratory conversation (stateless).
- **Phase 11 Validation Strategy: One-Time Multi-Drug Live Audit vs. Permanent High-Cost E2E Tests** — Phase 11 completed a deliberate one-time multi-drug live validation pass (KEYTRUDA/6mo, KEYTRUDA/2yr exercising chunking at scale, HUMIRA/6mo) against live openFDA + OpenAI APIs, rather than adding new permanent automated E2E tests. This locked design decision reflects a cost/coverage trade-off: permanent E2E tests against live FDA data add real API call overhead to every test run, while genuine pipeline exercising benefits most from a once-per-phase human-supervised manual pass. Unit tests (236 total, including new dedup/ranking coverage-gap tests) cover algorithm correctness deterministically; one-time multi-drug validation (documented in `docs/TODO.md` results table) covers end-to-end pipeline behavior and performance characteristics (e.g., chunking pagination, scale, `.docx` generation validity) with real data without the runtime tax. Acceptance: all automated tests pass, both `deduplication.py` and `ranking.py` at 100% statement coverage, and multi-drug live consolidations produce valid draft documents.
- **Phase 11 Guardrail Audit: Documented but Deliberately Unfixed Conversational Fabrication Finding** — The structured adversarial guardrail audit (detailed in `docs/GUARDRAIL_AUDIT.md`) confirmed that six of seven guardrails operate correctly, but discovered and explicitly chose not to fix one real, reproducible issue: in multi-turn conversation, the agent can occasionally answer follow-up questions using general pharmacological knowledge rather than the specific retrieved FAERS dataset, since the current conversational context carries only summary counts (signal count, chunk counts) not the full ranked signal list. This creates a theoretical risk that follow-up answers drift from the dataset on record. Root cause: `agent_tools.py` intentionally truncates the `consolidate_psur_tool` output to top 20 signals (and summary counts) to manage LLM context window; the full `ConsolidationResult` is captured via contextvars but not directly accessible to the LLM for bounded summary generation. Fix would require either (a) expanding the agent's context window allowance, risking latency/cost, or (b) threading full signal list into the agent context, risking similar issues. Product decision: scoped this finding as a known, documented item for a future update rather than engineering a partial fix in this phase. Guardrail enforcement in the primary UI flows (sidebar-triggered consolidation → ranked signal table, and `.docx` export) remains unaffected, since those retrieve data directly from `ConsolidationResult`, not narrated by the agent.
...
**Current counts:** 236 unit tests (all passing, no API keys; includes 3 new Phase 11 dedup/ranking coverage-gap tests), 20 E2E tests (all passing against live openFDA + OpenAI APIs, including Phase 2 PSUR retrieval, Phase 3 deduplication, Phase 6 consolidation, Phase 8 multi-turn agent, and Phase 9 document generation tests). Both `deduplication.py` and `ranking.py` at 100% statement coverage.
